...
└── [app-shell]                 — signed in
    ...
    │       ├── StreakStrip.svelte   — persistent weekly-streak / consistency strip
...
| File | Purpose |
|------|---------|
| auth.ts | Supabase auth: signIn, signUp (+ explicit emailRedirectTo = origin+BASE_URL), resendConfirmation, sendPasswordReset, updatePassword, signOut, onAuthChange (handles INITIAL_SESSION, SIGNED_IN, SIGNED_OUT, and PASSWORD_RECOVERY → recovery AuthState for the set-new-password screen). NB: emailRedirectTo is required — Site URL fallback drops the path → root 404 |
| storage.ts | bootstrapState, saveLocal, saveCloud — localStorage + Supabase sync |
| supabase.ts | Supabase client singleton + earliest-point PASSWORD_RECOVERY capture (isRecoveryPending/clearRecoveryPending) |
| errorTracker.ts | window.onerror + unhandledrejection → Supabase app_errors (deduped per session) |
...
│   │   ├── app.css
│   │   ├── main.ts
│   │   ├── components/   (24 components — incl. CopyDaySheet.svelte, StreakStrip.svelte)
...
│   │   ├── lib/          (dates, day-status, program, state-helpers, state-merge, exercise-sort, workout-metrics)
│   │   └── types/workout.ts
│   └── package.json
